...
class MyListRepository extends GetxController implements GetxService {
  ...
  Future<void> removeItemFromMyList(UserItemAddPayload payload, Function(Result result, UserItemModel? response, String? message) completion) async {
    try {
      final networkResponse = await network.loadHTTP(endpoint: Endpoints.userItemDelete, method: HTTPMethod.post, payload: NetworkPayload.userItemPayload(payload: payload));
      try {
