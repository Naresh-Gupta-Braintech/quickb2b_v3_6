...
class CartController
  ...
  templist =
      [];
  bool isCartFetchedSuccess =
      false;

  void makeEditable() {
    cartData?.data?.allInventories?.forEach(
      (
        order,
      ) {
        order.controller2?.text =
            order.quantity ??
            "";
        order.controller1?.text =
            order.measureQty ??
            "";
        print(
          "controller1 :: ${order.controller1?.text} controller2 :: ${order.controller2?.text}",
        );
      },
    );
    isEdit =
  ...
  void removedItem(
    ...
    cartData?.data?.allInventories?.removeAt(
      index,
    );

    CartData? localcartData =
        await LocalStorage.getCartDetails();
    AllInventory? inventory =
        localcartData?.data?.allInventories?[index];
    inventory?.quantity = 0.toString();
    inventory?.originQty = 0.toString();
    localcartData?.data?.allInventories?.removeAt(
      index,
    );
    await LocalStorage.saveCartDetails(
      localcartData,
    );
    Get.find<
          HomeController
        >()
        .removedItemFromCartForCartView(
          inventory,
        );
  ...
  void addItemToCartLocally({
    ...
    int
    index =
        home?.data?.allInventories?.indexWhere(
          (
            inventory,
          ) =>
              inventory.itemCode ==
    ...
    final orginQty =
        home?.data?.allInventories?[index].controller2?.text ??
        "";
    final measureQty =
        1;
    double qty =
    ...
    AllInventory inventory =
        home?.data?.allInventories?[index] ??
        AllInventory();
    inventory.uom =
        inventory.controller2?.text ??
        "";
    if (productAtIndexInCart ==
        -1) {
      cart?.data?.allInventories?.add(
        inventory,
      );
    } else {
  ...
  void addItemToCartLocall({
    required AllInventory product,
  }) async {
    CartData? cart =
        await LocalStorage.getCartDetails();

    final orginQty =
        product.controller2?.text ??
        "";
    final measureQty =
        1;
    double qty =
  ...
  void removeItemFromCardLocally({
    String? itemCode,
  }) async {
    CartData? cart =
        await LocalStorage.getCartDetails();
    cart?.data?.allInventories?.removeWhere(
      (
        inventory,
      ) =>
          inventory.itemCode ==
